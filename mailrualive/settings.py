...
class Settings:
    ...
    # locations
    IS_RUNNING = True
    log_location = localize_path("MailruAlive", "logs")
    database_location = localize_path("MailruAlive", "db")
    OS_ENV = (
    ...
    chromedriver_location = localize_path(
        "MailruAlive", "assets", specific_chromedriver
    )
    if not chromedriver_location or not path_exists(chromedriver_location):
        chromedriver_location = localize_path("MailruAlive", "assets", "chromedriver")

    # minimum supported version of chromedriver
    chromedriver_min_version = 2.36

    platform_name = "mailrualive"

    # set a logger cache outside the MailruAlive object to avoid
    ...
    WORKSPACE = {"name": "MailruAlive", "path": environmental_variables.get("HOME")}

    DATABASE_LOCATION = localize_path("MailruAlive", "db", "mailrualive.db")
